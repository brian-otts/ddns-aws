...
> = async (event) => {
  ...
  if (sourceIp !== ipAddress) {
    ...
    logger.warn(
      `${suspiciousActivityPrefix} - Source IP does not match provided IP`,
      { ipAddress }
    );
    await alert(
      `${suspiciousActivityPrefix} - IP Mismatch for ${hostname}`,
  ...
  try {
    ...
    const dnsRecordTtl = latest.dnsRecordTtl?.N
      ? parseInt(latest.dnsRecordTtl.N)
      : 600;
    ...
    if (!safeEqual(expectedHmac, validationHmac)) {
      ...
      await alert(
        `Auth Error for ${hostname}`,
        `Bad HMAC from IP: ${sourceIp}`
      );
      return respond(403, "Invalid HMAC");
    }

    // Get current A record
    const r53Resp = await r53.send(
    ...
    const currentR53Ip =
      r53Resp.ResourceRecordSets?.[0]?.ResourceRecords?.[0]?.Value;
